			...
			[:valid,		 'valid_token',	15.minutes.from_now]
				...
				describe "my request cookie token is #{has_request_token.to_s}," do
					before do
						...
						@member.stub!(:remember_token).and_return(token_value) 
						@member.stub!(:remember_token_expires_at).and_return(token_expiry)
						@member.stub!(:remember_token?).and_return(has_request_token == :valid)
					end
					...
					if (has_request_token == :valid)
						...
						it 'does refresh token'				do @member.should_receive(:refresh_token);		 do_create end 
						it "sets an auth cookie"			 do do_create;	end
					else
						it 'makes a new token'			 do @member.should_receive(:remember_me);			 do_create end 
						it "does not refresh token"	do @member.should_not_receive(:refresh_token); do_create end
						it "sets an auth cookie"			 do do_create;	end
					end
			end
		end
...
describe SessionsController do
	describe "route generation" do
		it "should route the new sessions action correctly" do
			route_for(:controller => 'sessions', :action => 'new').should == {:path => '/login', :method => :get}
		end
