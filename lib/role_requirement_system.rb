...
module RoleRequirementSystem
	...
	module RoleSecurityClassMethods
		...
		#		require_role "admin", :only => :update, :unless => "current_member.authorized_for_listing?(params[:id]) "
		#
		# Valid options
		#
		...
		# This is the core of RoleRequirement.	Here is where it discerns if a member can access a controller or not./
		def member_authorized_for?(member, params = {}, binding = self.binding)
			...
			self.role_requirements.each{| role_requirement|
				...
				passed = false
				roles.each { |role|
					passed = true if member.has_role?(role)
				} unless (! member || member==:false)
	...
	module RoleSecurityInstanceMethods
		...
		def access_denied
			if logged_in?
				...
				AuthenticatedSystem.instance_method(:access_denied).bind(self).call
			end
		end
		
		def check_roles
			return access_denied unless self.class.member_authorized_for?(current_member, params, binding)
			
			true
		end
		
	protected
		# receives a :controller, :action, and :params.	Finds the given controller and runs member_authorized_for? on it.
		...
		def url_options_authenticate?(params = {})
			...
			klass.member_authorized_for?(current_member, params, binding)
		end
	end
end